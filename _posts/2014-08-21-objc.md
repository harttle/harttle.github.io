...
objective c 属性与方法并无`public`与`private`的区别。类中声明的变量（无论是对象属性还是类属性）都是对外不可见的，外部可见的只有方法，`@propperty`的本质是生成访问器方法。`@interface{}`中声明的变量将被作为对象属性，`@interface{}`外禁止声明变量；`@implementation{}`中声明的变量也将被视为对象属性，`@implementation`外的变量将被作为类属性。

objective c 通过引入`.h`文件来使方法可见（即`public`）。在`.m`文件中除了`@implementation`外，通常会定义类的Extension（`@interface(){}`）。因为`.m`文件不会引入，该扩展中定义的方法只有当前类可见（即`private`），同样可以实现属性隐藏。
