...
属性可以声明为 `strong` 或 `weak`，用它们来组织对象图。要避免形成 `strong` 引用环，否则当它们的拥有者析构后，它们将同时保持 `alive`。有一些 Cocoa 和 Cocoa Touch 类不支持 `weak`，此时应使用 `unsafe unretained`，对象离开作用域后不再 `alive`，但指针不会设为 `nil`（unsafe）。
