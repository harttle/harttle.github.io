...
而POST或PUT数据必须放在消息主体（entity-body）中，这样的数据便是HTTP表单，表单数据的编码方式应在HTTP头中进行设置（Content-Type header字段），
常用的编码方式有：
...
当然这些数据格式完全可以自行定义。HTML支持前三种格式，可以通过`<form>`标签的`enctype`属性来进行设置。
...
这是HTML中`<form>`标签默认的编码方式，即`application/x-www-form-urlencoded`。如下的表单提交后产生的HTTP请求可能是这样的：
...
假设我们选择一个文件：`chrome.png`，表单提交后产生的请求可能是这样的：
...
其中`boundary`是用来分隔`entity body`中的每个字段。

### json

在POST时指定Content-Type为`application/json`，便会以json方式提交。这类表单通常是从ajax中产生的，例如：
