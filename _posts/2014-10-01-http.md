...
在web开发中最常见的莫过于GET和POST，其中GET一般将参数编码在url中（HTTP header）来传递数据；而POST数据必须放在消息主体（entity-body）中，根据需要可以采用不同编码方式（Content-Type header字段）：
...
在POST时指定Content-Type为`application/json`，便会以json方式提交：
